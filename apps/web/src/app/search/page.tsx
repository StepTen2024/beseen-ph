...
import { Search, MapPin, Star, Filter, SlidersHorizontal, X } from 'lucide-react';
...
const MOCK_RESULTS = [
  ...
  { id: 3, name: "Nayong Pilipino", category: "Park", city: "Clark", rating: 4.5, image: "https://images.unsplash.com/photo-1506905925346-21bda4d32df4?w=400", type: "place" },
  { id: 4, name: "Best Pizza in Angeles City", category: "Article", city: "Angeles City", image: "https://images.unsplash.com/photo-1565299624946-b28f40a0ae38?w=400", type: "article" },
];
        ...
        <div className="max-w-4xl mx-auto px-4 py-4">
          <div className="relative">
            ...
            {query && (
              <button 
                onClick={() => setQuery('')}
                className="absolute right-4 top-1/2 -translate-y-1/2 p-1 rounded-full hover:bg-slate-800"
              >
      ...
      <main className="max-w-4xl mx-auto px-4 py-6">
        ...
        <div className="space-y-4">
          {results.map((result) => (
            <Link
              key={result.id}
              href={result.type === 'article' ? `/articles/${result.id}` : `/${result.type}/${result.id}`}
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  <span className={`px-2 py-0.5 rounded text-xs font-medium ${
                    result.type === 'business' ? 'bg-emerald-500/10 text-emerald-400' :
                    result.type === 'place' ? 'bg-cyan-500/10 text-cyan-400' :
